...
// get movie recommendations from search 
export const getMovieRecommendationAction = (name) => async (dispatch) => {
  try {
    dispatch({type: moviesConstants.MOVIES_RECOMMENDATION_REQUEST});
    const respone = await moviesAPIs.getRecommnedationMovieService(name);
    dispatch( {
      type: moviesConstants.MOVIES_RECOMMENDATION_SUCCESS,
      payload: respone,
    })
  } catch (error) {
    ErrorsAction(error, dispatch, moviesConstants.MOVIES_RECOMMENDATION_FAIL);
  }
} 
